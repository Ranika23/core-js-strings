...
function extractNameFromTemplate(value) {
  const val1 = value.replace('Hello, ', '');
  return val1.replace('!', '');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (upperChar.indexOf(str[i]) >= 0) {
      const charInd = upperChar.indexOf(str[i]);
      if (charInd + 13 > 25) {
        result.push(upperChar[charInd + 13 - 26]);
      } else {
        result.push(upperChar[charInd + 13]);
      }
    } else if (lowerChar.indexOf(str[i]) >= 0) {
      const charInd = lowerChar.indexOf(str[i]);
      if (charInd + 13 > 25) {
        result.push(lowerChar[charInd + 13 - 26]);
      } else {
        result.push(lowerChar[charInd + 13]);
      }
    } else {
      result.push(str[i]);
    }
